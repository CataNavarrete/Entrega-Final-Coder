...
def register (request): 
    if request.method == 'POST':
        ...
        #usuario_extendido, _ = MyUser.objects.get_or_create(user=request.user)
    ...
    if request.method == 'POST':  
        ...
        if form.is_valid():
            ...
            if user is not  None:
                ...
                return render(request, "indice/plantillas/index.html", {'msj':f'Welcome {usuario}!', 'user_avatar':buscar_url_avatar(request.user)})
            else:
...
def perfil(request):
    mas_datos, _ = MyUser.objects.get_or_create(user=request.user)
...
def buscar_url_avatar(user):
    ...
    usuario_extendido, _ = MyUser.objects.get_or_create(user=user)
